...
rownames(numeric_dom) <- c("Dominant Hand Father", "Dominant Hand Mother")

numeric_total = matrix(as.numeric(unlist(mtotal)),nrow=nrow(mtotal)) 
colnames(numeric_total) <- colnames(mtotal)
rownames(numeric_total) <- c("Total Father", "Total Mother")
...
This table is the mother and father grouped together, comparing the combined the number of touches done with the dominant hand and the non-dominant hand. This is the most important table because we plan to use the proportion from this table as our test statistic.

```{r test, echo = FALSE}
finished_table = t(t(rowSums(unf_table)))
colnames(finished_table) <- c("Father and Mother")
kable(finished_table) %>%
...
* Gather a much larger sample size around a wide variety of people
  + While our small sample size is strong evidence that these two people use their dominant hand more often, it is not fully representative of the population. It provides us a conjecture for how the population would behave, not at all certainity.
